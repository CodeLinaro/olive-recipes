...
This directory demonstrates the optimization of the [Llama3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) model using the QAIRT Pipeline API.

## Overview

The pipeline workflow uses the `QairtPipelinePass` to perform AdaScale weight optimization, GPTAQ weight quantization, model transformation, DLC conversion, and HTP compilation in a single YAML-recipe-driven pass.
...
A legacy workflow using `QairtPreparation + QairtGenAIBuilder` is also available — see [Legacy Workflow](#legacy-workflow) below.
...
For SC8480XP:
```bash
olive run --config htp_sc8480xp_pipeline.json
```

For SC8380XP:
```bash
olive run --config htp_sc8380xp_pipeline.json
```
...
The legacy workflow uses `QairtPreparation` (model quantization script) and `QairtGenAIBuilder` (HTP compilation).
...
### Legacy Preparation Instructions

1. Authenticate with Hugging Face (see step 1 above).
...
3. (Optional) Use qairt-vm to install a non-default version of QAIRT and set QAIRT_SDK_ROOT (see step 3 above).
