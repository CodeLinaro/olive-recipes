...
# # AIMET Quantization workflow for Llama 3.1 8B
# 
# This notebook shows a working code example of how to use AIMET to quantize Llama 3 family models

# ---
# ### Required packages
# The notebook assumes AIMET and LLamaV3 related packages are already installed.
...
if hasattr(__builtins__,'__IPYTHON__'):
    ...
    get_ipython().system('sudo -H apt-get install libtinfo5')
    get_ipython().system('sudo -H apt-get install -y clang')
    get_ipython().system('sudo -H apt-get install -y libc++-dev')
    get_ipython().system('sudo pip install aenum')
    get_ipython().system('sudo -H pip install --quiet --upgrade --root-user-action=ignore --no-cache-dir aimet-torch==2.14.0 --index-url=https://pypi.org/simple')
    get_ipython().system('sudo -H pip install --quiet --upgrade --root-user-action=ignore --no-cache-dir transformers==4.47.0')
    get_ipython().system('sudo -H pip install --quiet --upgrade --root-user-action=ignore --no-cache-dir tokenizers==0.21.0')
    get_ipython().system('sudo -H pip install --quiet --upgrade --root-user-action=ignore --no-cache-dir jinja2==3.1.0')
    get_ipython().system('sudo -H pip install --quiet --upgrade --root-user-action=ignore --no-cache-dir datasets==4.0.0')
...
parser = argparse.ArgumentParser(
    description='Llama 3.1 Quantization Script',
...
Required Variables (must be set):
  ...
  MODEL_ID                  Path to the Llama model directory
...
assert context_length <= 8273, "Context length longer than 8273 for llama3 model family has not been validated for accuracy"
...
# Allow direct specification via config, otherwise use dynamic path based on target
htp_config_file = get_config_value(
    'HTP_CONFIG_FILE',
    f'/usr/local/lib/python3.10/dist-packages/aimet_common/quantsim_config/htp_quantsim_config_{nsp_target.dsp_arch}.json'
)
...
#### temp code to see which aimet version is used
import aimet_torch
print(f'AIMET Version is : {aimet_torch.__version__}')
####
...
model_id = get_config_value("MODEL_ID", None, 'none')
...
# modified code to make it work with transformers 4.50
...
if apply_decoder_seqmse or apply_lm_head_seqmse:
    ...
    with event_marker("Instantiate wikitext Dataloaders"):
        seqmse_wiki_train_dataloader, _, _ = get_wiki_dataset(seqmse_dataloader_length, tokenizer, cache_dir, path = get_config_value("WIKI_DATASET_PATH", None, 'none'))
