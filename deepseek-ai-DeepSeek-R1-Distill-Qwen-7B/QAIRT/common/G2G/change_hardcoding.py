...
def apply_fix(onnxmodel, base_dir, fix):
    ...
    def fix_tensor_proto_in_initializer(tensor_proto):
        tensor = numpy_helper.to_array(tensor_proto, base_dir=base_dir)
        ## Adjusted tensor handles two cases
        ##   1. Add node for padding: where initializer is of form [0,1,2,...,EXPORT_ARN-1]
        ##   2. Min node for SWA: where initializer has form [EXPORT_SCL-EXPORT_ARN, SCL-EXPORT_ARN + 1, ..., EXPORT_SCL-1]
        ## For both the case, adjusted_tensor would come out to be of form [0,1,2,...,EXPORT_ARN-1]
        adjusted_tensor = tensor - tensor[0]
        if len(tensor.shape) == 1 and tensor.shape[0] in fix and (adjusted_tensor == np.arange(tensor.shape[0])).all():
            new_shape = [fix[i] if i in fix else i for i in tensor.shape]
            print(f'{tensor_proto.name} {tensor.shape}, => {new_shape}')
            ## Now new_offset will be fix.get(tensor[0], tensor[0])
            ## which is 0 in case of Add node, and scl-arn (from fix dict) in case of Min node
            arr = fix.get(tensor[0], tensor[0]) + np.arange(new_shape[0], dtype=tensor.dtype)
            new_tensor = numpy_helper.from_array(arr, tensor_proto.name)
            changed[0] += 1
            return new_tensor
