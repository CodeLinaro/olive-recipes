...
class MHA2SHAOptimizer:
    ...
    def lora_b_query_split(self, info_dict, num_heads, head_dim):
        ...
        lora_query_node = info_dict["query"]["mha_lora_node"] if "mha_lora_node" in info_dict["query"] else None
        query_lora_split = None
        if init_name not in info_dict["query"] and lora_query_node and lora_query_node.lora_b.op_type == "MatMul":
...
and expect when transposing C to dim=2 will have H in NCHW transposed to dim = 1 or 3. Can't determine head_num.")
    ...
    def get_mha_pattern_info(self, d_qkv, qk_matmul_node, qkv_matmul_node):
        ...
        mha_base_attn_node = BaseAttnNode(
            ...
            maskedsoftmax_reducemin = None,
            maskedsoftmax_add = None,
            maskedsoftmax_where = None,
        )
        nodes_list = []
        ...
        while j < len(self.pattern) - 1:
            ...
            elif self.pattern[j:j+4] == ['ReduceMin', 'Add', 'Where', 'Softmax']:
                reducemin_node = self.get_node_by_input_name[nodes_list[-1].output[0]][0]
                assert reducemin_node.op_type == 'ReduceMin'
                add_node = self.get_node_by_input_name[reducemin_node.output[0]][0]
                assert add_node.op_type == 'Add'
                where_node = self.get_node_by_input_name[add_node.output[0]][0]
                assert where_node.op_type == 'Where'
                nodes_list.append(reducemin_node)
                nodes_list.append(add_node)
                nodes_list.append(where_node)
                mha_base_attn_node.maskedsoftmax_reducemin = reducemin_node
                mha_base_attn_node.maskedsoftmax_add = add_node
                mha_base_attn_node.maskedsoftmax_where = where_node
    ...
    def create_sha_pattern(self, info_dict, ns, head_num, value_inp, sha_nodes_list, sha_base_attn_node_list):
        ...
        while j < len(self.pattern) - 1:
            ...
            # Attention mask
                ...
                # masked softmax pattern, handle reducemin-add-where in the same time
                mha_mask_reducemin = info_dict["nodes_list"][j]
                mha_mask_add = info_dict["nodes_list"][j+1]
                mha_mask_where = info_dict["nodes_list"][j+2]

                assert mha_mask_reducemin.op_type == "ReduceMin"
                assert mha_mask_add.op_type == "Add"
                assert mha_mask_where.op_type == "Where"

                mha_reducemin = info_dict["nodes_list"][j]
                attr_names = [x.name for x in mha_reducemin.attribute]
                keepdims = 0
                if "keepdims" in attr_names:
                    keepdims = onnx.helper.get_node_attr_value(mha_reducemin, "keepdims")
                axes = None
                if "axes" in attr_names:
                    axes = onnx.helper.get_node_attr_value(mha_reducemin, "axes")
                sha_reducemin = self._op_factory.get_reduce_min_op(sha_nodes_list[-1], 
                                                                axes=axes, keepdims=keepdims)[0]
                sha_mask_add = self._op_factory.get_add_op(sha_reducemin, mha_mask_add.input[1], propose_op_name="masked_softmax_min_Add")
                sha_mask_where = self._op_factory.get_where_op(mha_mask_where.input[0], sha_nodes_list[-1].output[0],
                                                               sha_mask_add.output[0])

                sha_nodes_list.append(sha_reducemin)
                sha_nodes_list.append(sha_mask_add)
                sha_nodes_list.append(sha_mask_where)
                sha_base_attn_node_list.maskedsoftmax_reducemin.append(sha_reducemin)
                sha_base_attn_node_list.maskedsoftmax_add.append(sha_mask_add)
                sha_base_attn_node_list.maskedsoftmax_where.append(sha_mask_where)
                self.model.graph.node.extend([sha_reducemin, sha_mask_add, sha_mask_where])
                j += 2
            else:
