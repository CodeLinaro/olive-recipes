...
def scan_previous_nearst_candidate_node(output2node, start_values, valid_types, skip_map=None, get_node_type=None):
    check_values = start_values[:]
    yield_nodes = set()
    while len(check_values) > 0:
        candidate_v = check_values.pop(0)
        if candidate_v in output2node:
            candidate_node = output2node[candidate_v]
        else:
            # maybe initializer
            continue

        if candidate_node.name in yield_nodes:
            continue

        if get_node_type is not None:
            op_type = get_node_type(candidate_node)
        else:
            op_type = candidate_node.op_type

        if op_type in valid_types:
            yield_nodes.add(candidate_node.name)
            yield candidate_node
        elif skip_map is not None and op_type in skip_map:
            # skip current node
            check_values += [candidate_node.input[x] for x in skip_map[op_type]]
    return
...
class QuickAttentionFinder:
    ...
    def get_attn_info(self):
        ...
        skip_map = {
            # key: op_type, value: searching input ids of this op
            "Div":[0,1], "Sub":[0,1], "Add":[0,1], "Mul":[0,1],
            "Reshape":[0], "Transpose":[0], "Squeeze":[0], "Unsqueeze":[0], "Cast":[0],
            "Where":[1], # for MaskedSoftmax
        }

        # search pattern  ["MatMul", "Softmax", "MatMul"]
        ...
        for node in self.model.graph.node:
            ...
            # bottom-up search from Matmul
            if module_type != "MatMul":
                continue

            candidate_matmul1 = node
            for candidate_softmax in scan_previous_nearst_candidate_node(self.get_node_by_output_name, 
                                                                         [candidate_matmul1.input[0]],
                                                                         valid_types=["Softmax"], skip_map=skip_map,
                                                                         get_node_type=get_node_type):
                for candidate_matmul0 in scan_previous_nearst_candidate_node(self.get_node_by_output_name, 
                                                                            candidate_softmax.input,
                                                                            valid_types=["MatMul"], skip_map=skip_map,
                                                                            get_node_type=get_node_type):
                    mha_mapper.reset()
                    mha_mapper.is_use = True
                    mha_mapper.mha_potential = []
                    mha_mapper.MatMul_nodes.append(candidate_matmul0)
                    mha_mapper.MatMul_nodes.append(candidate_matmul1)
                    mha_mapper.mha_potential.append(candidate_matmul0)
                    mha_mapper.mha_potential.append(candidate_softmax)
                    mha_mapper.mha_potential.append(candidate_matmul1)
                    mha_mapper.expecting_module_type_idx = 2
