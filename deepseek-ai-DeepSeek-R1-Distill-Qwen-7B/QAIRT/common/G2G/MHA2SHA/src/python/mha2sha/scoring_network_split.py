...
from mha2sha.utils.encoding_mapper_utils import AimetEncodingVersion
...
class ScoringNetworkSpliter:
    def __init__(
        ...
        self.encoding_version = AimetEncodingVersion(self.encodings["version"])
    ...
    def split_batch_matmul(self):
        ...
        if self.encoding_version == AimetEncodingVersion.V1:
            new_activation_encodings = []

            for key, value in encodings_to_map.items():
                # find source activation encoding
                for encoding in self.encodings['activation_encodings']:
                    if encoding['name'] == key:
                        copied_encodings = encoding.copy()
                        copied_encodings.pop('name')
                        # for each destination, copy and append the encoding
                        for v in value:
                            copied_encodings_with_name = copied_encodings.copy() | {'name': v}
                            new_activation_encodings.append(copied_encodings_with_name)
            
            split_encodings['activation_encodings'] = new_activation_encodings + self.encodings['activation_encodings']
