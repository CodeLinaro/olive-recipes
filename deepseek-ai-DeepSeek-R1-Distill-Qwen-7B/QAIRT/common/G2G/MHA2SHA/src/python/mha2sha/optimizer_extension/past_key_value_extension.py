...
class PastKeyValueExtension:
    ...
    def reshape_cache_position_to_4d(self, cache_position):
        ...
        :param cache_position: cache position [input_seq_len]
        ...
        try:
            ...
            try:
                ...
                while indices_tensor_name not in self.mha2sha_optim.mha_model_input_names_index_dict.keys():
                    ...
                    if parent_node.op_type == "Min":
                        last_min_op_node = parent_node

                cache_position = parent_node
                self.cache_index_name = cache_position.input[0]

                # If swa cache_position is computed inside the graph, this logic needs to create separate cache_position
                nodes_driven_from_cache_position = self.mha2sha_optim.get_node_by_input_name[cache_position.output[0]]
                if last_min_op_node and (last_min_op_node in nodes_driven_from_cache_position):
                    cache_position = last_min_op_node
                    self.cache_index_name += "_min"
                
                # Only need to find cache position once when not sure if the model use native KV (ScatterElement)
                if not self.cache_index_name in self.cache_position:
                    self.cache_position[self.cache_index_name] = self.reshape_cache_position_to_4d(cache_position)
    ...
    def transform_cache_position_to_indices(
      self, head_dim
        ...
        mul_repeat_node, Mul_repeat_init = self.mha2sha_optim._op_factory.get_mul_op(
            self.cache_position[self.cache_index_name],
            mul_value,
            "MulRepeat"
        )
    ...
    def concat_past_key_value_input_with_scatter(
        ...
        if not self.cache_index_name in self.indices:
            self.indices[self.cache_index_name] = self.transform_cache_position_to_indices(head_dim) # [1, 1, head_dim, seq_len]

        k_axis = self.k_scatter_node.attribute[0].i
        scatter_key_node = self.mha2sha_optim._op_factory.get_scatter_element_op(
            past_key_node, self.indices[self.cache_index_name], key_inp, k_axis
        )
        ...
        indices_transposed = self.mha2sha_optim._op_factory.get_transpose_op(
            self.indices[self.cache_index_name], [0, 1, 3, 2]
