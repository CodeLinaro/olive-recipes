...
class OpFactory:
    ...
    def get_reduce_min_op(
        ...
        if not output:
            output, self._tensor_name_set = create_tensor_name(
                "ReduceMin", self._tensor_name_set
            )
        node_name, self._node_name_mapping_dict = create_node_name(
            self._model.graph, "ReduceMin", self._node_name_mapping_dict
        )

        reduce_min_node = make_node(
            "ReduceMin",
            inputs=[inp],
            outputs=[output],
            axes=axes,
