...
def _np(t):
    if isinstance(t, torch.Tensor):
        ...
        # 如果是 tuple/list，递归处理第一个元素
        return _np(t[0]) if len(t) > 0 else np.array([])
...
class TestVectorBase:
    ...
    def prepare_dummy_inputs(self, onnxmodel, missing_tensors):
        ...
        missing.update({i.name:i for i in onnxmodel.graph.input if i.name in missing_tensors})
        missing.update({i.name:i for i in onnxmodel.graph.output if i.name in missing_tensors})
        ...
        for name, info in missing.items():
            ...
            dummy = np.zeros(shape)  # Use zeros to make SQNR 0dB
...
class TracerTestVector(TestVectorBase):
    ...
    def get_batches(self, list_of_names: List[List[str]]) -> List[List[np.array]]:
        def _is_kv_in_out(name):
            return re.search("_(key|value)_\d+_(in|out)$", name) != None
        def _get_kv_idx(list_):
            return list_.index("key") if "key" in list_ else list_.index("value")
        ...
        for batch_index, picklefile in enumerate(self.picklefiles):
            ...
            # Updated list of names will contains all input and output names with past_<key/value>_<in/out>_<layer> replaced with <key/value>_<in/out>_<layer>.
            # This is done in order to support other key/values input (other than past, for eg. swa_<key/value>_<in/out>_<layer>)
            updated_list_of_names = [['_'.join(name.split('_')[_get_kv_idx(name.split('_')) : ]) if _is_kv_in_out(name) else name for name in names] for names in list_of_names]
            not_found =  [name for names in updated_list_of_names for name in names if name not in loaded]
            if not_found:
            ...
            assert all(name in loaded for names in updated_list_of_names for name in names), \
                f'Not found: {[name for names in updated_list_of_names for name in names if name not in loaded]}'
            yield [[loaded[name] for name in names] for names in updated_list_of_names]
    ...
    def _flatten_and_rename(self, batch):
        ...
        def _add_mapping(name, torch_name, tensor):
            """populate a dictionary with mapping from ONNX name -> Torch Name"""
            self.intermediate_tensor_mapping[name] = {'name':torch_name, 'shape':f"{list(tensor[0].shape)}"}
        ...
        #     # 确保 actual_tensor 是 numpy array 或 torch tensor
        #     if isinstance(actual_tensor, torch.Tensor):
        #         shape = list(actual_tensor.shape)
        #     elif isinstance(actual_tensor, np.ndarray):
        #         shape = list(actual_tensor.shape)
        #     else:
        #         # 如果还是 tuple，继续解包
        #         if isinstance(actual_tensor, (tuple, list)):
        #             actual_tensor = actual_tensor[0]
        #         shape = list(actual_tensor.shape) if hasattr(actual_tensor, 'shape') else []
            
        #     self.intermediate_tensor_mapping[name] = {'name':torch_name, 'shape':f"{shape}"}


        for key, value in batch.items():  # for each node in the pkl file
            if self.pickle_type == "npy":
                if key == 'inputs':
                    yield from [(k, v) for k, v in value.items()]
                elif key == 'outputs':
                    yield from [(k, v) for k, v in value.items()]
                elif key == 'intermediate_tensors':
                    yield from [(k, v) for k, v in value.items()]
                ...
                # per-layer inputs, injected for testing purposes in the AIMET pipeline
                continue
            ...
            elif key == 'swa_position_ids' and isinstance(value, (tuple, list)):
                # Handle RoPE embedding, untuple and rename
                yield from [(f'swa_position_ids_cos', value[0]), (f'swa_position_ids_sin', value[1])]
                ...
                if isinstance(value[0][0], (tuple, list)):
                    ...
                    for layer in range(num_layers):
                        yield from [(f'key_{layer}_h{head}_in', v) for head, v in enumerate(value[layer][0])]
                        yield from [(f'value_{layer}_h{head}_in', v) for head, v in enumerate(value[layer][1])]
                else:
                    yield from [(f'key_{i}_in', v[0]) for i, v in enumerate(value)]
                    yield from [(f'value_{i}_in', v[1]) for i, v in enumerate(value)]

            elif key == 'output_key_values' or key == 'output_values':
                assert isinstance(value, (tuple, list)), f'Unexpected output_key_values type: {type(value)}'

                # if we have anchor values in the output_kv test vectors values[0] = output_kv, value[1] = anchor_buffer_out
                if len(value) == 2 and isinstance(value[0], (tuple, list)) and isinstance(value[0][0], (tuple, list)):
                    yield from [(f'key_{i}_out', v[0]) for i, v in enumerate(value[0])]
                    yield from [(f'value_{i}_out', v[1]) for i, v in enumerate(value[0])]
                    yield from [(f'anchor_buffer_{i}_out', v) for i, v in enumerate(value[1])]

                # if we don't have anchor values
                else:
                    if isinstance(value[0][0][0], (tuple, list)):
                        value = value[0]

                    if isinstance(value[0][0], (tuple, list)):
                        num_layers, num_heads = len(value), len(value[0][0])
                        for layer in range(num_layers):
                            yield from [(f'key_{layer}_h{head}_out', v) for head, v in enumerate(value[layer][0])]
                            yield from [(f'value_{layer}_h{head}_out', v) for head, v in enumerate(value[layer][1])]
                    else:
                        yield from [(f'key_{i}_out', v[0]) for i, v in enumerate(value)]
                        yield from [(f'value_{i}_out', v[1]) for i, v in enumerate(value)]

            # Anchor buffer input for the long context base model
            elif key == 'anchor_buffer' :
                # we expect one tensor per layer
                assert isinstance(value, (tuple, list)), f'Unexpected (input) anchor_buffer type: {type(value)}'
                yield from [(f'anchor_buffer_{i}_in', v) for i, v in enumerate(value)]

            elif isinstance(value, dict):
                # intermediate tensor dumps

                # if the tensor name in the pkl is not already mapped, find the closest matching ONNX name
                ...
                if key in self.test_vector_name_to_node_name:
                    ...
                    if 'input' in value:
                        ...
                        if node.op_type == 'Cast':
                            # Because QNN converter fuses 'Cast' into its input node,
                    ...
                    # if pkl.node = output of some node
                    if 'output' in value:
                        # assuming output[0]

                        _add_mapping(node.output[0], f'{key}.output', value['output'])
                        yield (f'{node.output[0]}', value['output'])

                        # the nodes that consume the output of this node
                        consumers = self.consumers[node.output[0]]

                        # if the only consumer is a Cast operation
                        ...
                        # if the node is a Concat and the only consumer is a Transpose operation
                        elif node.op_type == 'Concat' and len(consumers) == 1 and consumers[0].op_type == 'Transpose':
                            # the output of the consumer node = pkl.node.output
                            _add_mapping(f'{consumers[0].output[0]}', f'{key}.output', value['output'])
                            yield (f'{consumers[0].output[0]}', value['output'])
                else:
                    assert 'input' in value and len(value['input']) == 1, f"Unexpected intermediate tensor dump:{key}"
                    _add_mapping(key, f'{key}.input', value['input'])
                    yield (key, value['input'])

            # unexpected format in the pickle file
            else:
...
def get_split_tensors(onnxfile, onnxmodel=None, include_first_input=True, include_all_adds=False):
    '''
    Model topology in general case
            │ ←─────────  layers[0]  ────────────→ │       │ ←─────────  layers[-1]  ─────────────→ │
            │                                      │       │                                        │
    ...
    def get_nodes():
        ...
        seq = {id(node): idx for idx, node in enumerate(nodes)}
        producers = {tensor: node for node in model.graph.node for tensor in node.output}
        return nodes, seq, producers

    def can_produce(input_node, output_node):
        '''
        Check if input_node can be found in the producer chain
        '''
        if input_node is None or output_node is None:
            return False
        if _seq(input_node) > _seq(output_node):
            return False

        stack, visited = deque([output_node]), set()
        while stack:
            cur = stack.pop()
            if cur == input_node:
                return True
            visited.add(_seq(cur))
            for next_node in [producers[tensor] for tensor in cur.input if tensor in producers]:
                if _seq(next_node) not in visited and _seq(next_node) >= _seq(input_node):
                    stack.append(next_node)
        return False

    def _producer(tensor):
        while producers[tensor].op_type == 'Cast':
            tensor = producers[tensor].input[0]
        return producers[tensor]

    def _get_parent_nodes(tensor):
        x, y = [
            _producer(inp_tensor) if inp_tensor in producers else None
            for inp_tensor in _producer(tensor).input
        ]
    ...
    def depends_on_tensor(graph_input_tensor: str, node) -> bool:
        if node is None: return False
        stack, seen = deque([node]), set()
        while stack:
            cur = stack.pop()
            sid = _seq(cur)
            ...
            seen.add(sid)
            for t in cur.input:
                if t == graph_input_tensor:
                    return True
                if t in producers:
                    stack.append(producers[t])
        return False


    def get_chain_of_adds(nodes):
        '''
        In LLMs, all layers have just one long chain of residual add operations
        '''
        cache = {}

        # for the following case (occurs for LoRA v1, first attn block)
        #     where add_1, add_2, add_3 satisfy the "residual add" property
        #           z -- y --
        #          / \       \
        #        /    ------ add_2
        #  add_1                  \
        #      \                   add_3
        #       -----------------/
        # we want {add_1, add_3} to be identified
        #     and not {add_2, add_3}
        #     because add_2 is NOT an actual residual add (as per architecture)
        #
        # in dfs, we accept the child node's dfs result only if the current node
        #     also produces the other input (to the child)
        def _dfs(node):
            if id(node) in cache:
                return cache[id(node)]

            node_output = node.output[0]
            if node_output not in consumers:
                return "", [node_output]

            if node.op_type == "Add":
                parent_x, parent_y = _get_parent_nodes(node_output)
                is_residual = can_produce(parent_x, parent_y)

                if not is_residual:
                    # 如果某一侧没有 producer（图输入），检查另一侧是否依赖该输入张量
                    in_x, in_y = producers[node_output].input  # 这两个是原始输入张量名
                    if parent_x is None and in_x is not None:
                        is_residual = depends_on_tensor(in_x, parent_y)
                    elif parent_y is None and in_y is not None:
                        is_residual = depends_on_tensor(in_y, parent_x)

                if not is_residual:
                    return parent_x, []
            else:  # Cast node
                parent_x = _producer(node.input[0])

            longest = []
            for consumer in consumers[node_output]:
                consumer_parent, path = _dfs(consumer)
                if consumer_parent != node and consumer_parent!="":
                    continue
                if len(path) > len(longest):
                    longest = path

            res = [node_output] + longest
            cache[id(node)] = (parent_x, res)
            return parent_x, res

        all_adds = [node for node in nodes if node.op_type in ("Add")]
        consumers = defaultdict(list)
        for node in all_adds:
        ...
        return [tensor for tensor in max_path if producers[tensor].op_type == "Add"]


    def get_layer0_input(add0_output):
        add = producers[add0_output]
        a, b = [_producer(tensor) if tensor in producers else None for tensor in add.input]
        if a is None: return b
        if b is None: return a
        return add.input[0 if _seq(a) < _seq(b) else 1]

    nodes, seq, producers = get_nodes()
    residual_add_outputs = get_chain_of_adds(nodes)

    output_tensors = []
    if include_first_input:
        layer0_input = get_layer0_input(residual_add_outputs[0])
        output_tensors.append(layer0_input)
    if not include_all_adds:
        output_tensors += residual_add_outputs[1::2]
...
def _load_model(onnxfile, load_external_data=False, model_cache={}):
    if onnxfile not in model_cache:
        ...
        onnx_model = onnx.load(onnxfile, load_external_data=load_external_data)
        model_cache[onnxfile] = onnx.shape_inference.infer_shapes(onnx_model)
...
def _load_encoding(encodingfile, no_merge=False):
    ...
    if encodingfile is not None:
        ...
        if isinstance(quant_encoding_dict['param_encodings'], dict):
            ...
            all.update({enc['name']: enc for enc in quant_encoding_dict['activation_encodings']})
            all.update({enc['name']: enc for enc in quant_encoding_dict['param_encodings']})
        else:
            assert False, f'Unknown encoding format : param_encodings type:{quant_encoding_dict["param_encodings"]}'
...
def get_embedding_table_and_proto_and_node(onnxmodel, base_dir):
    embedding_table_node = [node for node in onnxmodel.graph.node if node.op_type == 'Gather'][0]
...
def concat_ssd_param_embeddings_into_embeddings_table(embedding_table, ssd_param_filename):
    ...
    print(f"SSD: the number of forecast tokens = {len(forecast_token_embeddings)}")

    new_embedding_table = np.concatenate((embedding_table, forecast_token_embeddings), axis=0)
    return new_embedding_table

def embed_forecast_token_embeddings(onnxmodel, ssd_param_filename, base_dir):

    embedding_table, embedding_table_proto, _ = get_embedding_table_and_proto_and_node(onnxmodel, base_dir)

    new_embedding_table = concat_ssd_param_embeddings_into_embeddings_table(embedding_table, ssd_param_filename)
...
def quantize(f, encoding, bw=8):
    def _round(x):
        sign = np.where(x < 0, -1, 1).astype(np.float32)
        return np.floor(np.abs(x) + 0.5) * sign

    def _quantize(f, scale, offset, dtype=eval(f"np.uint{bw}")):
    ...
    if isinstance(encoding, list):
        ...
        assert False, f'Unknown encoding format: {type(encoding)}'
...
def dump_embedding_table(onnxmodel, base_dir, encodingfile=None, output_dir='.'):
    def _save_emebedding_table(embedding_table, embedding_table_filename):
        embedding_table.tofile(os.path.join(output_dir, embedding_table_filename))

    embedding_table, _, embedding_node = get_embedding_table_and_proto_and_node(onnxmodel, base_dir)
    embedding_output_name = embedding_node.output[0]

    # Dump float embedding table data
    _save_emebedding_table(embedding_table, "embedding_table.bin")

    # Dump quantized embedding table data
    if encodingfile and os.path.exists(encodingfile):
        encodings = _load_encoding(encodingfile)
        assert embedding_output_name in encodings, f"encodings of {embedding_output_name} is not found in {encodingfile}"

        encoding = encodings[embedding_output_name]
        if isinstance(encoding, list):
            bw = encoding[0]['bitwidth']
        elif isinstance(encoding, dict):
            bw = encoding['bw']
        else:
            assert False, f'Unknown encoding format: {type(encoding)}'

        with open(os.path.join(output_dir, "embedding_table_quantized_param.json"), "w") as f:
            f.write(json.dumps(encoding, indent=4))

        quantized_embedding_table, _ = quantize(embedding_table, encoding, bw=bw)
        _save_emebedding_table(quantized_embedding_table, "quantized_embedding_table.bin")

def save_kv_cache(kvcache, encodings, filename, num_layers=10000,check_sqnr=True):
    def _is_kv_in(layer, name):
        return re.search(f'_(key|value)_{layer}_in$', name) != None

    # layerwise_kv_in_names contains the names of key_in/values_in from encodings keys. It is sorted to keep [key_in, value_in] order
    # for eg. layerwise_kv_in_names = [[swa_key_0_in, swa_value_0_in], [swa_key_1_in, swa_value_1_in], [past_key_2_in, past_value_2_in], ...]
    layerwise_kv_in_names = [sorted([name for name in encodings.keys() if _is_kv_in(layer, name)]) for layer in range(num_layers)]
    key_value_encodings = [[encodings[name] for name in kv_in_name] for kv_in_name in layerwise_kv_in_names]
...
def process_ssd_param(onnxfile, encodingfile, ssd_param_filename, transposed_key_cache=True, output_dir='.'):
    ...
    print(f"SSD: the length of the forecast prefix = {len_prefix}")
    ...
    if 'input_ids' in onnx_input_names:
        embed_forecast_token_embeddings(onnxmodel, ssd_param_filename, base_dir=os.path.dirname(onnxfile))
    else:
        assert any(name in onnx_input_names for name in ('inputs_embeds', 'input_embeds')), \
            f"input_ids and inputs_embeds/input_embeds are not found in the onnx input: {onnx_input_names}"
        print("Skipping embed_forecast_token_embeddings as embeddings table is not present in the model")

    num_layers = len([i for i in onnxmodel.graph.output if re.search("_value_\d+_out$", i.name) != None])
...
def split_onnx_by_names(onnxfile, modelname, pickle_filedir, *list_of_output_tensors, output_dir='.', onnxmodel=None, encoding_file=None, embed_ssd_params_file=None, using_qairt_workflow=False, dump_embedding_table_data=False, num_splits=None):

    if embed_ssd_params_file:
    ...
    if dump_embedding_table_data:
        dump_embedding_table(onnxmodel, os.path.dirname(onnxfile), encodingfile=encoding_file, output_dir=output_dir)
    ...
    for i, subgraph in enumerate(splitter.split(list_of_output_tensors)):
        ...
        submodel.ir_version = onnxmodel.ir_version
    ...
    for source in ['qt']:
        ...
        for i, (new_basename, input_names, output_names)  in enumerate(new_model_info):
            ...
            if i > 0:
                ...
                input_names = [i for i in input_names if i not in ['input_ids',]]
...
def _get_embedding_sizes(onnxmodel):
    "Get dimensions of the Embedding : "
    embedding_weight_name = next(node.input[0] for node in onnxmodel.graph.node if node.op_type=="Gather" and "input_ids" in node.input)
    embedding_weight, = [i for i in onnxmodel.graph.initializer if embedding_weight_name == i.name]
    vocab_size, embedding_size = embedding_weight.dims
...
def get_encoding(encodings, in_tensor):
    if isinstance(encodings, list):
        ...
        if in_tensor in encodings:
            return encodings[in_tensor].copy()
    return None

def fill_input_encodings_of_split(onnxmodel, encoding_file, output_tensor_list, lora_importer_config=None):
    producer = {tensor: node for node in onnxmodel.graph.node for tensor in node.output}
    def _fill_input_encodings_of_split(encodingfile):
        changed = False
    ...
    _fill_input_encodings_of_split(encoding_file)
    if lora_importer_config:
        with open(lora_importer_config, "r") as f:
            lora_importer_data = yaml.safe_load(f)
        assert "use_case" in lora_importer_data, f"'use_case' is not found in {lora_importer_config}"
        for use_case in lora_importer_data["use_case"]:
            assert "quant_overrides" in use_case, f"'quant_overrides' is not found in {use_case}"
            _fill_input_encodings_of_split(use_case["quant_overrides"])
...
def split_onnx(onnxfile, modelname, pickle_filedir, num_splits, output_dir='./', split_embedding=False, split_lmhead=False, encoding_file=None, embed_ssd_params_file=None, using_qairt_workflow=False, lora_importer_config=None, splitting_points=None, has_lm_head=True):
    ...
    # get hidden_states_size from lm_head
    hidden_states_size, vocab_size = _get_lm_head_sizes(onnxmodel)
    if not has_lm_head:
        # get hidden_states_size from embeddings
        hidden_states_size, vocab_size = _get_embedding_sizes(onnxmodel)
    # if in Eaglet model, get hidden_states_size from last_hidden_states
    last_hidden_states = None
    for output in onnxmodel.graph.output:
        if "last_hidden_states" == output.name:
            last_hidden_states=output
            hidden_states_size = output.type.tensor_type.shape.dim[-1].dim_value
            break
    print(f'Using per-layer output shape: {[batch_size, seq_length, hidden_states_size]}')
    tensor_shape_map = {name:  [batch_size, seq_length, hidden_states_size] for name in output_tensor_list}
    ...
    names_to_split = []
    if isinstance(split_embedding, str):
        output_tensor_list[0] = split_embedding
        print(f'Using split_embedding with tenosr: {split_embedding}')
        # Mark the split_embedding flag as True
        split_embedding = True
    if split_embedding:
        first_output_tensors = output_tensor_list[0].split(',')
        if encoding_file:
            fill_input_encodings_of_split(onnxmodel, encoding_file, first_output_tensors, lora_importer_config=lora_importer_config)
        names_to_split.append(output_tensor_list[0])
        output_tensor_list.pop(0)

    num_layers = len(output_tensor_list)
    remaining_splits = num_splits - (1 if split_embedding else 0) - (1 if split_lmhead else 0)
    assert remaining_splits > 0, f"No of remaining splits {remaining_splits=} must be greater than 0 after split_embedding and split_lmhead"

    def _is_anchor_buffer(layer, name):
        return re.search(f'anchor_buffer_{layer}_', name) != None

    past_anchor_values = {layer:[output for output in output_names if  _is_anchor_buffer(layer, output) ] for layer in range(num_layers)}

    split_points = list(range(0, num_layers, num_layers//remaining_splits))[1:]
    
    num_extra_layers = num_layers - remaining_splits*(num_layers//remaining_splits)
    if num_extra_layers > 0:
        extra_layers = list(range(1, num_extra_layers+1)) + [num_extra_layers]*(len(split_points) - num_extra_layers)
        split_points = [i+j for i,j in zip(split_points, extra_layers)][:-1]
    # if the lm_head needs to be in a separate split (last split)
    if split_lmhead and has_lm_head:
        split_points += [num_layers]
    
    if splitting_points:
        split_points = splitting_points

    layer_start = 0
    for layer_end in split_points:
        outputs = [output_tensor_list[layer_end-1]]
        if last_hidden_states and layer_end == num_layers:
            outputs = [last_hidden_states.name]
        for layer in range(layer_start, layer_end):
            outputs += past_key_values[layer]
            outputs += past_anchor_values[layer]
        names_to_split.append(','.join(outputs))
        layer_start = layer_end


    per_layer_output_value_info = [onnx.helper.make_tensor_value_info(name, onnx.TensorProto.FLOAT, tensor_shape_map[name]) for name in tensor_shape_map.keys()]
    onnxmodel.graph.value_info.extend(per_layer_output_value_info)

    print('Names_to_split\n', '\n\n'.join(names_to_split))
    if split_lmhead and not has_lm_head:
        print(f"WARNING! {has_lm_head=} means LM_HEAD is not there in the model but {split_lmhead=} is given. Hence, num of splits will be 1 less.")
        assert num_splits - 1 == len(names_to_split)+1, f"Failed to split into {num_splits} pieces!"
    else:
    ...
    return split_onnx_by_names(onnxfile, modelname, pickle_filedir, *names_to_split,
                    ...
                    using_qairt_workflow=using_qairt_workflow, dump_embedding_table_data=split_embedding,
