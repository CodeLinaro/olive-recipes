...
class OnnxSplitter:
    ...
    def partition_subgraph(self,
        ...
        # Remove the output tensors that has been included in the current subgraph
        self.graph_outputs.difference_update({output.name for output in new_outputs if output.name in self.graph_outputs})
    ...
    def split(self, list_of_intermediate_output_tensors):
        ...
        for i, output_tensors in enumerate(list_of_intermediate_output_tensors):
            ...
            if self.verbose: print(f'Partitoin new graph: {graphname} for outputs[{output_tensors}]')
