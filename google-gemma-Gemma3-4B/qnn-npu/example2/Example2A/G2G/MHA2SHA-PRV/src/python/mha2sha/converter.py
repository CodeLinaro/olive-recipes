...
class MHA2SHAConverter:
    ...
    def _run_optimizations(
        ...
        if self._lora_alpha_from_input:
            self._lora_alpha = prequant_opt.lora_alpha



        if self._skip_mha2sha:
            log_info("Step 5.2: Skip mha2sha model adaption.\n")
        ...
        log_info("Saving lora weights to safe tensor...")
        lora_tensor_name_to_init_dict = {name:value for name, value in out_model_init_dict.items()
                                         if "lora" in name} 
        if lora_tensor_name_to_init_dict:
            save_init_to_safetensor(lora_tensor_name_to_init_dict, self._sha_export_path)
            
        if self._model_name is None:
            output_model_name = self._model_or_path.split(".")[-2].split('/')[-1]
        ...
        ou.save_model(mha2sha_model, onnx_output_filename)
