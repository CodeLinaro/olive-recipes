...
try:
    ...
    aimet_version = None
if aimet_version is None or aimet_version > version.Version('2.0.0'):
...
def apply_input_output_exception(quantizer, exception):
    ...
    if quantizer is None:
        if exception.get("data_type", "int") == "float":
            return FloatQuantizeDequantize(dtype = torch.float16)
        else:
            return QuantizeDequantize(shape=(), bitwidth=exception.get("bitwidth", 16), symmetric= exception.get("asymmetric", True) is False)
