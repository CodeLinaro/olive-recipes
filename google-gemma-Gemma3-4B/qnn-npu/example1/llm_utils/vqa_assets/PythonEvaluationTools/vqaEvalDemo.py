...
import os
import sys
# dataDir = f"{os.path.dirname(os.path.abspath(__file__))}/../results"
# sys.path.insert(0, '%s' % (dataDir))
...
def vqaEvalDemo(resFile, annFile, quesFile, imgDir, outDir):
    # create vqa object and vqaRes object

    # Collect all question_ids in resFile
    qidxs = [entry['question_id'] for entry in json.load(open(resFile))]

    vqa = VQA(annFile, quesFile, qidxs=qidxs)
    ...
    [resFile, accuracyFile, evalQAFile, evalQuesTypeFile, evalAnsTypeFile] = ['%s/%s%s_%s_%s_%s.json' % (outDir, versionType, taskType, dataType, dataSubType,
    ...
    print('evalQA to retrieve low score result...')
    ...
    if len(evals) > 0:
        ...
        print('\n')
        print('generated answer (accuracy %.02f)' %
              (vqaEval.evalQA[randomEval]))
        ann = vqaRes.loadQA(randomEval)[0]
        print("Answer:   %s\n" % (ann['answer']))

        imgId = randomAnn[0]['image_id']
        imgFilename = 'COCO_' + dataSubType + \
            '_' + str(imgId).zfill(12) + '.jpg'
    ...
    plt.xticks(range(len(vqaEval.accuracy['perQuestionType'])),
               vqaEval.accuracy['perQuestionType'].keys(), rotation=90, fontsize=5)
    plt.title('Per Question Type Accuracy', fontsize=10)
    plt.xlabel('Question Types', fontsize=10)
    plt.ylabel('Accuracy', fontsize=10)
    plt.savefig(f'{outDir}/vqa_v2_eval.png')
    plt.close()
