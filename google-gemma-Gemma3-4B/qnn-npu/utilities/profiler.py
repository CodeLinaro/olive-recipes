...
class EventProfiler:
    ...
    def _launch_nsp_process(self, process_name):
        ...
        self._htp_proc = subprocess.Popen(
            f"{ADB} push ../../utilities/get_on_target_peak_ram.sh /data/local/tmp/;"
    ...
    def _run_nsp_process(self):
        ...
        while not self._stop_event.is_set():
            if proc_stdout_poll.poll(5000):
                intermediate_response = self._htp_proc.stdout.readline().decode().strip()
                # Update response only if intermediate_response is non-empty:
                if intermediate_response:
                    response = intermediate_response

        peak_ram_in_kb = int(response.split("PEAK :")[-1].split("(DMA")[0].strip())
